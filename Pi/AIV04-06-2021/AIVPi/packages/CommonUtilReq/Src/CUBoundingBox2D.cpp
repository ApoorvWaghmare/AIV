...
double BoundingBox2D::GetMinDistanceBetweenBoxes(const BoundingBox2D &box)const
{
	if (IsIntersecting(box))
		return -1.0;

	double deltaX = 0., deltaY = 0. , dist = 0. ;

	if (m_maxX < box.GetMinX()) // Other box is to right
	{
		deltaX = box.GetMinX() - m_maxX;
	}
	else if (m_minX > box.GetMaxX()) //Other box is to the left
	{
		deltaX = m_minX - box.GetMaxX();
	}
	else // x overlap
		deltaX = 0.0; 

	if (m_maxY < box.GetMinY()) // Other box is on the top
	{
		deltaY = box.GetMinY() - m_maxY;
	}
	else if (m_minY > box.GetMaxY()) //Other box is below
	{
		deltaY = m_minY - box.GetMaxY();
	}
	else // y overlap
		deltaY = 0.0;
